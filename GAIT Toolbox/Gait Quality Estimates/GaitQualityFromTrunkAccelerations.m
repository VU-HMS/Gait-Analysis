function [MeasuresStruct] = GaitQualityFromTrunkAccelerations(AccData, FS, LegLength, CutoffFreq, varargin)
%% function [MeasuresStruct] = GaitQualityFromTrunkAccelerations (AccData, FS, LegLength, CutOffFreq, varargin)
%
% INPUT
...
% CutOffFreq: Cutoff frequency for high pass Butterworth filter (used to
%             counteract integration drift)
...
% 2022-10 (RC): Add input parameter CutoffFreq. StrideFrequency now
%               taken from StrideSpectralMeasures() rather than
%               StrideTimeMeasures(). Latter function renamed to
%               StrideRegularityMeasures(). Order of calculations adjusted 
...
t=tic;
for nn = 1 : ntot
    MeasuresStruct = StrideSpatialMeasures(MeasuresStruct, AccData, FS,...
                     ApplyRealignment, LegLength, CutoffFreq);
end
if (ntot > 1)
...
%% Stride regularity measures
if (verbosityLevel > 1)
    prLog("Calculate stride regularity measures.\n", fncName);
end
t=tic;
for nn = 1 : ntot    
    [MeasuresStruct] = StrideRegularityMeasures(MeasuresStruct, AccData, FS);
end
if (ntot > 1)
    printf('Time to calculate stride regularity measures = %.3f seconds.\n', toc(t)/ntot);
end
